...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
	...
	{
		// Adel
		if (AccountUtil.getEnableAccountsCount() > 0)
		{
			AccountUtil.logoutCurrentAccount();
			return;
		}
		...
		editor.clear().commit();
		editor = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "emoji", Activity.MODE_PRIVATE).edit();
		editor.putLong("lastGifLoadTime", 0).putLong("lastStickersLoadTime", 0).commit();
		editor = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "mainconfig", Activity.MODE_PRIVATE).edit();
			...
			public void run()
			{
				...
				if (!updatesOnMainThread.isEmpty())
				{
					...
					for (int a = 0; a < updatesOnMainThread.size(); a++)
					{
						...
						{
							SharedPreferences.Editor editor2 = ApplicationLoader.applicationContext.getSharedPreferences(ApplicationLoader.instance_number + "emoji", Activity.MODE_PRIVATE).edit();
